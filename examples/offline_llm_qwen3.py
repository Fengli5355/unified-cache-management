...
def setup_environment_variables():
    ...
    path_to_dataset = os.getenv(
        "DATASET_PATH", "/home/fl/LongBench/data/multifieldqa_zh.jsonl"
    )
    if not os.path.isfile(path_to_dataset):
        path_to_dataset = input(
            "Enter path to one of the longbench dataset, e.g. ./fl/LongBench/data/multifieldqa_zh.jsonl: "
        )
        if not os.path.isfile(path_to_dataset):
...
def build_llm_with_uc(module_path: str, name: str, model: str):
    ...
    kv_transfer_config = {
        ...
        "kv_connector_extra_config": {
            "UCM_CONFIG_FILE": os.path.join(
                os.path.dirname(__file__), "ucm_config_asu.yaml"
            )
        },
    }
