...
Status KvTestConfigLoader::Load(const std::string& configPath, KvTestConfig& config) const
{
    ...
    try {
        GetStringAny(values, {"kv.key_prefix", "key_prefix"}, config.keyPrefix);

        GetUint64Any(values, {"kv.value_size", "value_size"}, config.valueSize);
        GetUint64Any(values, {"kv.seed", "seed"}, config.seed);
        GetUint64Any(values, {"kv.count", "count"}, config.count);

        GetUint32Any(values, {"limits.batch_store_max", "batch_store_max"},
                     config.limits.batchStoreMax);
        GetUint32Any(values, {"limits.batch_retrieve_max", "batch_retrieve_max"},
                     config.limits.batchRetrieveMax);
        GetUint32Any(values, {"limits.delete_max", "delete_max"}, config.limits.deleteMax);
        GetUint32Any(values, {"limits.exist_max", "exist_max"}, config.limits.existMax);

        GetUint64Any(values, {"bench.io_size", "io_size"}, config.bench.ioSize);
        GetUint32Any(values, {"bench.concurrency", "concurrency"}, config.bench.concurrency);
        GetUint64Any(values, {"bench.duration_sec", "duration_sec"}, config.bench.durationSec);
        GetUint64Any(values, {"bench.warmup_sec", "warmup_sec"}, config.bench.warmupSec);
        GetUint32Any(values, {"bench.read_ratio", "read_ratio"}, config.bench.readRatio);
        GetUint32Any(values, {"bench.write_ratio", "write_ratio"}, config.bench.writeRatio);
        GetUint32Any(values, {"bench.batch_size", "batch_size"}, config.bench.batchSize);

        GetStringAny(values, {"output.path", "output_path"}, config.output.path);
        GetUint64Any(values, {"output.realtime_file_max_bytes", "realtime_file_max_bytes"},
                     config.output.realtimeFileMaxBytes);

        std::uint64_t timeoutMs{0};
        if (GetUint64Any(values, {"connection.timeout_ms", "timeout_ms"}, timeoutMs)) {
