...
Status BuildBenchData(const KvTestConfig& config, std::uint64_t keyCount, GeneratedData& data)
{
    ...
    data = GeneratedData{};
    data.keys.reserve(keyCount);
    data.values.reserve(keyCount);
                        ...
                        AsuClientRunner& clientRunner, CommandResult& result) const
{
    ...
    if (bench.readRatio > 100 || bench.writeRatio > 100 ||
        bench.readRatio + bench.writeRatio > 100) {
        return Status::Error(kExitInvalidArgument,
    ...
    auto runPhase = [&](std::uint64_t durationSec, bool collectStats) -> Status {
        ...
        while (Clock::now() < phaseEnd) {
            for (std::uint32_t inFlight = 0; inFlight < bench.concurrency; ++inFlight) {
                const auto begin =
                ...
                auto opStatus = ExecuteBenchOperation(
                    ...
                    currentEntryCount, operationIndex, config.asuClientConfig.defaultWaitTimeoutMs,
                    operationResult);
                ...
                measuredLatenciesUs.push_back(latencyUs);
                ++result.benchMetrics.completedOperations;
                result.benchMetrics.completedEntries += currentEntryCount;
                result.benchMetrics.completedBytes += currentEntryCount * bench.ioSize;
                ++windowOperationCount;
                windowEntryCount += currentEntryCount;
                windowBytes += currentEntryCount * bench.ioSize;
                windowLatencyUs += latencyUs;
